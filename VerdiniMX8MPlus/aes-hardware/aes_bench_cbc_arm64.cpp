...
void processImageFile(const char *filename, int iterations = 1) {
    ...
    // Krypter bildedata iterations ganger
    for (int i = 0; i < iterations; i++) {
        ...
        // Vis framgang for hver 100. iterasjon
        if (i > 0 && i % 100 == 0) {
            std::cout << "." << std::flush;
            if (i % 500 == 0) {
                std::cout << " " << i << " iterasjoner fullført\n";
            }
        }
    }
    auto t1 = std::chrono::high_resolution_clock::now();
    
    // Dekrypter data iterations ganger
    for (int i = 0; i < iterations; i++) {
    ...
    // Verifiser dekryptering en gang (ikke for hver iterasjon)
    ...
    // Beregn gjennomstrømning (basert på totalt prosessert data)
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        if (cmd == "REPEAT" && argc >= 4) {
            ...
            // Process image with iterations and return
            ...
            // Process image (with default 1 iteration) and return
