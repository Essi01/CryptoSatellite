...
public:
    ...
    bool begin()
    {
        if (!initialized)
        {
            std::cerr << "INA219 initialization failed: sensor files not accessible\n";
            return false;
        }
    ...
    bool readMeasurements(float &power_mW, float current_mA)
    {
        if (!initialized)
        {
            power_mW = 0;
...
void power_thread_fn()
{
    ...
    while (sampling.load())
    {
        ...
        if (ina.readMeasurements(power_mW, current_mA))
        {
