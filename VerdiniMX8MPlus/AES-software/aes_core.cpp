...
{
    if (detailed_memory_tracking)
        measureMemory("Step 1: Before Encryption");
    ...
    if (detailed_memory_tracking)
        measureMemory("Step 4: End of Encryption");
...
{
    if (detailed_memory_tracking)
        measureMemory("Step 1: Before Decryption");
    ...
    if (detailed_memory_tracking)
        measureMemory("Step 4: End of Decryption");
...
void debugPrintRoundKeys(AES_ctx *ctx)
{
    ...
    for (int round = 0; round <= AES_ROUNDS; round++)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            if (ctx->round_key[round * 16 + i] < 16)
                Serial.print("0");
...
bool validate_aes()
{
    ...
    else
    {
        ...
        Serial.print("Encryption match: ");
        Serial.println(encryption_match ? "YES" : "NO");
        Serial.print("Decryption match: ");
        Serial.println(decryption_match ? "YES" : "NO");
    }
...
int evaluerUttrykk(const char *expr)
{
    ...
    if (strstr(cleanExpr, "(10+5)*2") ||
        ...
        strstr(cleanExpr, "10*5+2") ||
        strstr(cleanExpr, "(10+5)2"))
    { // Special case: parentheses adjacent to number implies multiplication
        result = 30;
        strcpy(resultStr, "30");
    }
