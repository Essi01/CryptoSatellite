 ...
 * Using mbedtls with dual-core support for Portenta H7
 */
#include <Arduino.h>
...
// Pre-allocated encryption/decryption context for reuse
mbedtls_aes_context aes_enc_ctx;
mbedtls_aes_context aes_dec_ctx;
bool aes_contexts_initialized = false;
...
void initializeAesContexts() {
  if (!aes_contexts_initialized) {
    mbedtls_aes_init(&aes_enc_ctx);
    mbedtls_aes_init(&aes_dec_ctx);
    mbedtls_aes_setkey_enc(&aes_enc_ctx, aes_key, 128);
    mbedtls_aes_setkey_dec(&aes_dec_ctx, aes_key, 128);
    aes_contexts_initialized = true;
  }
}
...
void freeAesContexts() {
  if (aes_contexts_initialized) {
    ...
    aes_contexts_initialized = false;
  }
}
...
// OPTIMIZED: Core encryption/decryption functions now reuse AES context
unsigned long encrypt(const unsigned char* input, unsigned char* output, size_t len) {
  ...
  do {
    ...
    // Copy IV to temp since it gets modified during encryption
    unsigned char temp_iv[IV_SIZE];
    memcpy(temp_iv, iv, IV_SIZE);

    // Use pre-initialized context for encryption
    mbedtls_aes_crypt_cbc(&aes_enc_ctx, MBEDTLS_AES_ENCRYPT, len, temp_iv, input, output + IV_SIZE);

    // Extra work for consistent timing
    unsigned char small_iv[IV_SIZE];
    memcpy(small_iv, iv, IV_SIZE);
    mbedtls_aes_crypt_cbc(&aes_enc_ctx, MBEDTLS_AES_ENCRYPT, sizeof(extra_buffer), small_iv, extra_buffer, extra_buffer);

    end_time = micros();
  } while (((end_time - start_time) < MIN_ACCURATE_MICROS || iterations < MIN_ITERATIONS) && iterations < 20);
...
unsigned long decrypt(const unsigned char* input, unsigned char* output, size_t len) {
  ...
  do {
    ...
    // Use pre-initialized context for decryption
    mbedtls_aes_crypt_cbc(&aes_dec_ctx, MBEDTLS_AES_DECRYPT, len, iv, input + IV_SIZE, output);

    // Extra work for consistent timing
    unsigned char small_iv[IV_SIZE];
    memcpy(small_iv, iv, IV_SIZE);
    mbedtls_aes_crypt_cbc(&aes_dec_ctx, MBEDTLS_AES_DECRYPT, sizeof(extra_buffer), small_iv, extra_buffer, extra_buffer);
...
bool runImageTest(const char* filename, int iterations) {
  ...
  Serial.println("==========================================");
  Serial.print("Starting AES-CBC Hardware image test with ");
  Serial.print(iterations);
...
#ifdef USE_MULTICORE_RTOS
// RTOS thread functions - Updated to use A, W, J instead of mA, mW, mJ
void powerMeasurementThread() {
...
bool validate_aes() {
  ...
  Serial.println("==========================================");
  Serial.println("Validating AES-CBC hardware implementation...");
...
void setup() {
  ...
  SERIAL_NUCLEO.println("AES-HW-PORTENTA-READY");
