...
{
    printf("cipher_cfg = %d, cfg_128_64 = %d\n", cipher_cfg, cfg_128_64);
    fflush(stdout);

    if (cipher_cfg > cfg_256_128 || cipher_cfg < cfg_64_32)
    {
        printf("Invalid cipher_cfg: out of range\n");
        fflush(stdout);
        return 1;
    }
    ...
    printf("Before assigning function pointers\n");
    fflush(stdout);
    ...
    else if (cipher_cfg == cfg_72_48 || cipher_cfg == cfg_96_48)
    {
        cipher_object->encryptPtr = &Speck_Encrypt_48;
        cipher_object->decryptPtr = &Speck_Decrypt_48;
    }
    else if (cipher_cfg == cfg_96_64 || cipher_cfg == cfg_128_64)
    {
        cipher_object->encryptPtr = &Speck_Encrypt_64;
        cipher_object->decryptPtr = &Speck_Decrypt_64;
        printf("Matched cfg_128_64, setting Speck_Encrypt_64\n");
        fflush(stdout);
    }
    else if (cipher_cfg == cfg_96_96 || cipher_cfg == cfg_144_96)
    {
        cipher_object->encryptPtr = &Speck_Encrypt_96;
        cipher_object->decryptPtr = &Speck_Decrypt_96;
    }
    else if (cipher_cfg == cfg_128_128 || cipher_cfg == cfg_192_128 || cipher_cfg == cfg_256_128)
    {
    ...
    {
        printf("Default case hit: cipher_cfg = %d\n", cipher_cfg);
        fflush(stdout);
        return 1;
    }

    printf("Function pointers assigned successfully\n");
    fflush(stdout);

    printf("Returning 0 from Speck_Init\n");
    fflush(stdout);
    return 0;
}
...
{
    (*cipher_object.encryptPtr)(cipher_object.round_limit, cipher_object.key_schedule, (const uint8_t *)plaintext, (uint8_t *)ciphertext);
    return 0;
}
...
{
    (*cipher_object.decryptPtr)(cipher_object.round_limit, cipher_object.key_schedule, (const uint8_t *)ciphertext, (uint8_t *)plaintext);
    return 0;
}
